...
extern unsigned char getOneChar ();
...
tabinit()
{
	...
	limit = strlen(direct);
	for (i = 0; i < limit; ++i)
		mustshiftopt[(int)direct[i]] = mustshiftsafe[(int)direct[i]] = 0;
	limit = strlen(spaces);
	for (i = 0; i < limit; ++i)
		mustshiftopt[(int)spaces[i]] = mustshiftsafe[(int)spaces[i]] = 0;
	limit = strlen(optional);
	for (i = 0; i < limit; ++i)
		mustshiftopt[(int)optional[i]] = 0;
	limit = strlen(base64);
	for (i = 0; i < limit; ++i)
		invbase64[(int)base64[i]] = i;
...
ConversionResult OldConvertUTF7toUCS2(
...
UCS2*
LibEthConvertUTF7toUCS2 ()
{
	ConversionResult result = ok;
	DECLARE_BIT_BUFFER;
	int shifted = 0, first = 0, wroteone = 0, base64EOF, base64value, done;
	unsigned int c, prevc, tc;
	unsigned long junk;
	register UCS2 *target = (UCS2 *) calloc ( 1, sizeof(UCS2) );
	register UCS2 *targetEnd = target+1;
