...
#define GFF_UTIL_MINOR_VERSION 1
#define GFF_UTIL_EXPORT_DATE "Wed Dec 11 21:39:02 EET 1996"

extern char* ISO639_2Names[];   /* in etime.data */
...
#ifdef __STDC__
  boolean flag_reset ( int argc, char **argv, MyFlags** timeFlags );
...
long int  *year;
  ...
  flag_reset ( argc, argv, &timeFlags );
  ...
  if ( !timeFlags->noTime )
    {
      if ( timeFlags->date != NIL )        /* We are converting a date here   */
        {                                  /* so we do not want hours:min:sec */ 
          ...
          Time->tm_wday = GregorianToFixed ( Time->tm_mday, Time->tm_mon+1, Time->tm_year+1900 ) % 7;
          ...
          if ( timeFlags->calIn == eng )  /* Gregorian -> Ethiopic */
            {
              ...
              fprintf ( stdout, euroString );
              fprintf ( stdout, " is " );
              fprintf ( stdout, ethioString );
            }
          else                             /* Ethiopic -> Gregorian */
            {           
              // Time->tm_wday = EthiopicToFixed ( Time->tm_mday, Time->tm_mon+1, Time->tm_year+1900 ) % 7;
              Time->tm_wday = EthiopicToFixed ( Time->tm_mday, Time->tm_mon, Time->tm_year+1900 ) % 7;
              ...
              fprintf (stdout, " " );
              fprintf ( stdout, euroString );
              if ( timeFlags->LCInfo & WITHUTF8 )  /* check also language */
  ...
  if ( timeFlags->isHoliday || timeFlags->Talbot || timeFlags->YearName 
       || timeFlags->isLeapYear )
    {
      ...
      if ( timeFlags->date == NIL ) 
        ...
        {
          *date  = timeFlags->date;
          *month = timeFlags->month + 1;
          *year  = timeFlags->year + 1900;
        }
...
/*---------------- Define and Initialize Flag Values -------------------------*/
  ...
  *timeFlags              = (MyFlags *) malloc ( sizeof(MyFlags) );
  ...
  (*timeFlags)->noTime    = false;  /* when true we don't print dates or hours,
                                       only Holiday, Leap Year, Talbot, etc   */
  (*timeFlags)->calIn     = eng;    /* meaning Gregorian         */
  (*timeFlags)->calOut    = tir;    /* meaning Ethiopic-Tigrigna */
  (*timeFlags)->date      = NIL;
  (*timeFlags)->month     = NIL;
  (*timeFlags)->year      = NIL;
...
/* We go ahead and reset the calOut variable now to make life easier later
  ...
  if ( ++i < argc )  /* we found a -l or -lang request */
    {
	  int j;
      for (j = 0; j <= tir; j++ )
      	if ( !strcmp(argv[i], ISO639_3Names[j]) 
            || !strcmp(argv[i], ISO639_2Names[j]) )
		  {
        	(*timeFlags)->calOut = j;
			break;
		  }
  ...
  else if ( (envlang = (char *)getenv("LANG")) != NULL ) 
    if ( !strcmp(envlang, ISO639_3Names[amh]) 
         || !strcmp(envlang, ISO639_2Names[amh]) 
         || !strcmp(envlang, "Amharic") 
         || !strcmp(envlang, "amharic") 
       )
          ...
          switch ( toupper(argv[i][1]) ) 
            {
              ...
              case '1' :  (*timeFlags)->LCInfo |= WITHONEPREFIX;
                          break;
			  */
              case '1' :  (*timeFlags)->LCInfo |= WITHHOUR12;
                          break;
              ...
              case 'R' :  (*timeFlags)->LCInfo |= 0x0200;
                          break;
...
USAGE:
              case '?' :
              default  :  if ( ((*timeFlags)->LCInfo & WITHUTF8) 
                               || (i+1 < argc && argv[i+1][1] == 'u') ) 
                            {
                              if ( (*timeFlags)->calOut == amh )
                                printAmharicHelp ();
                              else if ( (*timeFlags)->calOut == tir )
                                printTigrignaHelp ();
                              exit (0); 
                            }
...
printEnglishHelp()
{
  ...
  fprintf (stderr, "\t   LibEth@EthiopiaOnline.Net\n\n");
